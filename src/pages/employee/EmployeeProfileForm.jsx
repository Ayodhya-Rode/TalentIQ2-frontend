...
import { createEmployeeProfile } from "../../api/employeeApi";
import { getCategories } from "../../api/categoryApi";

export default function EmployeeProfileForm({ onComplete }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await createEmployeeProfile({ ...form, categoryIds: selectedCategoryIds });
      onComplete();
    } catch (err) {
  ...
  return (
    <div className="max-w-2xl mx-auto bg-bg-card border border-border rounded-xl p-6">
      ...
      <p className="text-sm text-text-secondary mb-6">
        Candidates will see this information before booking a slot with you.
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm text-text-secondary mb-2">
            Categories you take interviews in
          </label>
